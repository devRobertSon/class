...
import { fetchJSON, fetchBytes, metaExists, sortWeeks, sortQuizzes, weekLabelOf, isoWeekId, isoWeekIdAfter, toYMD, homeworkShareText, formatBytes, ATTENDANCE, ATTENDANCE_ORDER, isNoShow } from "./store.js";
...
function renderMain() {
  ...
  if (S.roster) for (const a of S.roster.academies) normalizeWeekIds(a.fileId);
...
// ---------- ②½ 회차별 입력 (기본 탭) ----------
// 한 페이지에서 수업 흐름 순서대로: ① 지난 주 숙제 체크 → ② 지난 주 퀴즈 점수 →
// ③ 이번 주 출석 → ④ 진도 → ⑤ 이번 주 숙제 입력 → ⑥ 이번 주 퀴즈 등록
function renderWeeklyTab(container) {
  ...
  hwCard.appendChild(
    el("p", { class: "hint", text: "학생별 체크는 다음 주 '회차별 입력'의 ①에서 하게 됩니다. (또는 '숙제 체크' 탭)" })
  );
  ...
  for (const q of thisQuizzes) {
    qCard.appendChild(
      el("div", { class: "student-row" }, [
        ...
        el("span", { class: "hint", text: `만점 ${q.max}점` }),
        el("span", { class: "s-actions" }, [
